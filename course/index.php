...
if(user_has_role_assignment($USER->id,12) && user_has_role_assignment($USER->id,13)){
    echo "<style>select[name='categoryid'] option {
    display: none;
}
ul.breadcrumb li:nth-child(2){
    display:none;
}
ul li.type_setting.depth_2.item_with_icon:nth-child(1) {
    display: none;
}
li.type_setting.depth_2.item_with_icon.current_branch:nth-child(1) {
    display: none;
} 
li.type_setting.depth_2.item_with_icon.current_branch {display:none}
</style>";
    }
echo $OUTPUT->footer();
