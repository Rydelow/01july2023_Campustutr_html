...
$PAGE->set_title('Tutors Enrole Courses');
$PAGE->set_heading('Tutors Enrole Courses');
$loginsite = 'Custom Home Page';
//$PAGE->navbar->add($loginsite);
$PAGE->navbar->add(('Tutors Enrole Courses'), new moodle_url('/blocks/customhomepage/tutors_role_course.php'));
$PAGE->set_url('/blocks/customhomepage/tutors_role_course.php');
echo $OUTPUT->header();
...
{
	$arra=array();
	$dataff=$DB->get_records_sql('SELECT * FROM {tutors_course}');
	 ...
	 foreach ($_POST['course_id'] as $key) 
	  {
		...
		$data=new stdClass();
		$data->course_id= $key;
		$dataff=$DB->get_record_sql('SELECT * FROM {tutors_course} where course_id='.$key);
		if(empty($dataff->id)){
		$DB->insert_record('tutors_course',$data,true);
		}
		...
		if(!empty($results)){
			...
			$sql=$DB->delete_records('tutors_course',array('course_id'=>$valuedelete));
			}
		}

	redirect($CFG->wwwroot."/blocks/customhomepage/tutors_role_course.php?sucessfully=update");	
}
...
foreach ($coursedata as $datavalue) {
  $dataf=$DB->get_record_sql('SELECT * FROM {tutors_course} where course_id='.$datavalue->id);
 ?>
