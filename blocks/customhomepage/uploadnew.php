...
$PAGE->set_title('User Csv Upload');
$PAGE->set_heading('User Csv Upload');
$loginsite = 'Custom Home Page';
//$PAGE->navbar->add($loginsite);
$PAGE->navbar->add(('User Csv Upload'), new moodle_url('/blocks/customhomepage/uploadnew.php'));
$PAGE->set_url('/blocks/customhomepage/uploadnew.php');
echo $OUTPUT->header();
...
$csv = array();
 ...
 $i=1;
                ...
                $row++;
                    if($row!=1){
                      ...
                      $u=preg_replace("/\([^)]+\)/","",str_replace(" ","",$usname));
                       ...
                       if(empty($data[3])){
                        $password=md5(str_replace(" ","",$data[1]));
                       }else{
                        $password=md5($data[3]);
                       }
        ...
        $userinsert->email= strtolower($data[2]);
...
$ndataavl=$DB->get_record_sql("select * from {user} where username='".$user_name."' or email='".strtolower($data[2])."'");
if(empty($ndataavl)){
  ...
  $erroremail[$row++]=strtolower($data[2]);
}
    ...
    if(empty($erroremail)){ 
    redirect($CFG->wwwroot.'/blocks/customhomepage/uploadnew.php?sucessfully=sucessfully');
    }
}    

if(!empty($erroremail)){

  \core\notification::error(implode(",",$erroremail).' are Exists');

\core\notification::success("Other data sucessfully Inserted");
  // redirect($CFG->wwwroot.'/blocks/customhomepage/uploadnew.php?sucessfully=sucessfully', implode(",",$erroremail).' are Exists', null, \core\output\notification::NOTIFY_ERROR);
}
