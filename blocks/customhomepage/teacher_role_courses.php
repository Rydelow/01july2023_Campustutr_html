...
$PAGE->set_title('Teacher Enrole Courses');
$PAGE->set_heading('Teacher Enrole Courses');
$loginsite = 'Custom Home Page';
//$PAGE->navbar->add($loginsite);
$PAGE->navbar->add(('Teacher Enrole Courses'), new moodle_url('/blocks/customhomepage/choose_your_role.php'));
$PAGE->set_url('/blocks/customhomepage/choose_your_role.php');
echo $OUTPUT->header();
...
{
	$arra=array();
	$dataff=$DB->get_records_sql('SELECT * FROM {teacher_courses}');
	 ...
	 foreach ($_POST['course_id'] as $key) 
	  {
		...
		$data=new stdClass();
		$data->course_id= $key;
		$dataff=$DB->get_record_sql('SELECT * FROM {teacher_courses} where course_id='.$key);
		if(empty($dataff->id)){
		$DB->insert_record('teacher_courses',$data,true);
		}
		...
		if(!empty($results)){
			...
			$sql=$DB->delete_records('teacher_courses',array('course_id'=>$valuedelete));
			}
		}

	redirect($CFG->wwwroot."/blocks/customhomepage/teacher_role_courses.php?sucessfully=update");	
}
...
foreach ($coursedata as $datavalue) {
  $dataf=$DB->get_record_sql('SELECT * FROM {teacher_courses} where course_id='.$datavalue->id);
 ?>
